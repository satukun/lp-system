...
## フォームのコア禁止エリア（絶対に触れないこと）

> マーケターが「デザインを変えて」と依頼した場合でも、以下のエリアは変更禁止。
> フォームのバリデーション・送信処理はエンジニアが管理する領域であり、
> Claudeが誤って書き換えると本番のフォームが破壊されるリスクがある。

### 触ってはいけないファイル・コードブロック

| ファイル / 箇所 | 理由 |
|---|---|
| `src/lib/formCore/` 以下（将来作成予定） | フォームのバリデーション・送信ロジックのコア。エンジニア管轄 |
| `htmlGenerator.ts` 内の `<form>` タグ・`action` 属性・`required` 属性 | 送信先・バリデーション設定。マーケター依頼では変更不可 |
| `htmlGenerator.ts` 内の `<input type="hidden">` タグ | APIキーやエンドポイントが含まれる可能性がある |
| S9セクションのフォーム送信処理に関わるJavaScript | バリデーション・送信・エラーハンドリングのロジック |

### 許可される変更範囲（マーケター依頼で対応可能）

- フォームセクション（S9）の**見た目・レイアウト**の変更
- フォームの**ラベルテキスト**（「お名前」「メールアドレス」など表示文字）
- フォームセクションの**背景色・余白・フォントサイズ**

### 設定変更が必要な場合の対応

フォームの送信先・必須項目・バリデーションルールを変更したい場合は、
**エンジニアに依頼すること**。将来的には `formConfig`（Q&A形式のUI）で
マーケターがコードを触らずに設定できる仕組みを整備予定。
