...
> 現状のS9フォームは `<form>` タグのHTMLを出力しているだけで送信先が存在しない。機能させるには外部への接続先が必要。まず①の形で構築し、将来的に②の独立サービスへ切り出す方針。

| 優先度 | 実装内容 | 概要 | ステータス |
|---|---|---|---|
| ① | **CLAUDE.md にフォーム禁止エリアを追記** | フォームのコアロジック（バリデーション・送信処理）に関わるファイル・コードブロックを Claude の変更禁止エリアとして明示する | 完了 |
| ② | **LPData型に `formConfig` を追加** | ウィザードにフォーム設定ステップを追加。管理者アドレス・CC・必須項目・送信先URLをマーケターが設定できるようにする | 未着手 |
| ③ | **htmlGenerator のフォーム出力を設定ベースに変更** | ハードコードされた `<form>` を `formConfig` から動的生成するよう変更。Formspree 等の外部サービスも接続可能にする | 未着手 |
| ④ | **PRレビューフロー整備（初期フェーズ）** | マーケターの変更 → PR → エンジニアが確認・マージ のフローを確立する。慣れてきたら自動化を検討 | 未着手 |
| ⑤ | **フォームだけ別サービスとして切り出す（将来）** | Q&A形式でフォームHTMLを生成・ダウンロードできる独立したWebサービスとして分離する | 未着手 |
