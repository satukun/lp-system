...
| 優先度 | 実装内容 | 概要 | ステータス |
|---|---|---|---|
| ① | **Figma → ウィザード同期の仕組み** | Figma MCP でデザインを読み取り、セクション・パーツをウィザード画面に自動反映できる仕組みを構築する | 未着手 |
| ② | **Claude → Figma 書き込み対応** | Pattern Overview / LP Preview への自動フレーム生成。現在は社内許可待ちのため保留 | **保留**（社内許可待ち）|

> **Cloudflare** はインフラ検討候補として保留。本番公開のタイミングで改めて判断する。
...
> Notion 連携・Cloudflare は対象外（除外済み / 保留）
